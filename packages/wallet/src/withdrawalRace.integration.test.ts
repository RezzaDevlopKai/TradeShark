...
async function seedWithdrawal(amount: string) {
  ...
  await client.pool.query(
    ...
    [
      ...
      userId,
      assetId,
      `race-available:${availableId}`,
      lockedId,
      `race-locked:${lockedId}`,
      pendingId,
      `race-pending:${pendingId}`,
      externalId,
      `race-external:${externalId}`
    ]
  );
  await client.pool.query(
...
async function cleanup(seed: Awaited<ReturnType<typeof seedWithdrawal>>) {
  if (!client) return;
  await client.pool.query(
    `DELETE FROM journal_entries WHERE transaction_id IN (SELECT id FROM journal_transactions WHERE reference_id = $1 OR reference_type = 'wallet_withdrawal_race_seed')`,
    [seed.withdrawalId]
  );
  await client.pool.query(
    `DELETE FROM journal_transactions WHERE reference_id = $1 OR idempotency_key = $2`,
    [seed.withdrawalId, seed.seedKey]
  );
  await client.pool.query(`DELETE FROM idempotency_keys WHERE key LIKE $1 OR key = $2`, [
    `withdrawal:${seed.withdrawalId}:%`,
    seed.seedKey
  ]);
  ...
  await client.pool.query(`DELETE FROM ledger_balance_projections WHERE account_id = ANY($1::uuid[])`, [
    [seed.availableId, seed.lockedId, seed.pendingId, seed.externalId]
  ]);
...
integration("PostgreSQL withdrawal lifecycle race integration", () => {
  ...
  it("serializes concurrent approval and failure into exactly one terminal outcome", async () => {
    if (!client) throw new Error("DATABASE_URL is required");
    ...
    try {
      ...
      const balances = await client.pool.query(
        ...
        [[seed.availableId, seed.lockedId, seed.pendingId]]
      );
  ...
  it("does not allow submission to race past a concurrent failure with an inconsistent ledger", async () => {
    if (!client) throw new Error("DATABASE_URL is required");
