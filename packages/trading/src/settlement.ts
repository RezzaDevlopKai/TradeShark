...
export async function settleTradeInTransaction(
  tx: TradeTransaction,
  ...
  if (parseScaled(buyerLimitPrice) < parseScaled(price)) throw new Error("buyerLimitPrice cannot be below execution price");
  ...
  if (accounts.length !== new Set(accountIds).size) throw new Error("All trade settlement ledger accounts must exist");

  const accountById = new Map(accounts.map((account) => [account.id, account]));
  const requireAccount = (id: string, assetId: string, accountType: "USER_AVAILABLE" | "USER_LOCKED" | "FEE_REVENUE") => {
    const account = accountById.get(id);
    if (!account || account.assetId !== assetId || account.accountType !== accountType) throw new Error(`Invalid settlement ledger account ${id}`);
    return account;
  };
  ...
  if (!buyerLockedQuote.userId || buyerLockedQuote.userId !== buyerAvailableBase.userId) throw new Error("Buyer settlement accounts must belong to the same user");
  if (!sellerLockedBase.userId || sellerLockedBase.userId !== sellerAvailableQuote.userId) throw new Error("Seller settlement accounts must belong to the same user");
  if (buyerLockedQuote.userId === sellerLockedBase.userId) throw new Error("Trade settlement cannot use the same buyer and seller user");

  if (releasedQuoteAmount !== "0.000000000000000000") {
    if (!input.buyerAvailableQuoteAccountId) throw new Error("buyerAvailableQuoteAccountId is required when a quote reservation must be released");
    requireAccount(input.buyerAvailableQuoteAccountId, market.quoteAssetId, "USER_AVAILABLE");
    if (accountById.get(input.buyerAvailableQuoteAccountId)?.userId !== buyerLockedQuote.userId) throw new Error("Buyer quote release account must belong to the buyer");
  }
  ...
  const quoteEntries = [
    ...
    ...(feeAmount !== "0.000000000000000000" ? [{ accountId: input.feeRevenueQuoteAccountId, direction: "credit" as const, amount: feeAmount }] : []),
    ...(releasedQuoteAmount !== "0.000000000000000000" ? [{ accountId: input.buyerAvailableQuoteAccountId!, direction: "credit" as const, amount: releasedQuoteAmount }] : [])
  ];
  ...
  const quote = await postJournalInTransaction(tx, {
    ...
    referenceId: input.tradeId,
    metadata: { marketId: input.marketId, tradeId: input.tradeId, asset: "quote", feeRate, buyerLimitPrice, releasedQuoteAmount },
...
export async function settleTrade(db: TradeSharkDatabase, input: SettleTradeInput): Promise<SettleTradeResult> {
  return db.transaction((tx) => settleTradeInTransaction(tx, input));
}
