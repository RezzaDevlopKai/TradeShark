...
async function seed(accountId: string, treasuryId: string, amount: string, key: string) {
  await postJournal(client!.db, {
    ...
    idempotencyKey: key,
    referenceType: "execution_edge_seed",
    entries: [
...
integration("PostgreSQL execution edge integration", () => {
  ...
  it("settles a sell taker against the best buy and charges the buyer-side fee", async () => {
    ...
    try {
      ...
      expect(execution.trades[0]?.price).toBe("12.000000000000000000");
      expect(execution.trades[0]?.quantity).toBe("1.000000000000000000");
      expect(execution.trades[0]?.feeAmount).toBe("0.066000000000000000");
      expect(execution.trades[0]?.releasedQuoteAmount).toBe("0.000000000000000000");
      expect(await balance(buyerQuoteLocked)).toBe("0.000000000000000000");
      expect(await balance(buyerQuoteAvailable)).toBe("37.934000000000000000");
      ...
      expect(await balance(sellerQuoteAvailable)).toBe("12.000000000000000000");
      expect(await balance(feeRevenue)).toBe("0.066000000000000000");
    } finally {
  ...
  it("leaves an unmatched order open with its reservation intact", async () => {
    ...
    try {
      ...
      expect(execution.remainingQuantity).toBe("2.000000000000000000");
      expect(execution.trades).toHaveLength(0);
      expect(await balance(buyerQuoteLocked)).toBe("20.110000000000000000");
      expect(await balance(buyerQuoteAvailable)).toBe("4.890000000000000000");
      ...
      expect(await balance(buyerQuoteLocked)).toBe("0.000000000000000000");
      expect(await balance(buyerQuoteAvailable)).toBe("25.000000000000000000");
    } finally {
  ...
  it("rolls back execution when the fee revenue account is missing", async () => {
    ...
    try {
      ...
      await client.pool.query(`DELETE FROM ledger_accounts WHERE id = $1`, [feeRevenue]);
      ...
      expect(persisted.rows[0]).toEqual({ status: "open", remaining_quantity: "1.000000000000000000" });
      expect(trades.rows).toHaveLength(0);
      expect(await balance(buyerQuoteLocked)).toBe("10.055000000000000000");
      expect(await balance(sellerBaseLocked)).toBe("1.000000000000000000");
    } finally {
      await client.pool.query(`DELETE FROM ledger_balance_projections WHERE account_id = $1`, [feeRevenue]);
      await client.pool.query(`DELETE FROM ledger_accounts WHERE id = $1`, [feeRevenue]);
